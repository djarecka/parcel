...
Pprof_list = ["Pprof_hydro_const_rhod", "Rhoprof_hydro_const_rhod", "Pprof_hydro_const_th_rv", "Rhoprof_hydro_const_th_rv"]
...
def test_pressure(dt):
    ...
    # running parcel model for different ways to solve for pressure  ...
        parcel(dt = dt, pprof = pprof, outfile = "test_"+ pprof+".nc") 
    ...
    for pprof in Pprof_list:
      ...
      z = f.variables["z"][:]
      plots[0].plot(f.variables["p"][::10] / 100.   , z[::10], style[i])
      plots[1].plot(f.variables["th_d"][::10]       , z[::10], style[i])
      plots[2].plot(f.variables["T"][::10]          , z[::10], style[i])
      plots[3].plot(f.variables["rhod"][::10]       , z[::10], style[i])
      plots[4].plot(f.variables["r_v"][::10] * 1000 , z[::10], style[i])
      plots[5].plot(
	f.variables["RH"][::10]                     , z[::10], style[i], 
	[f.variables["RH"][:].max()] * z.shape[0], z, style[i]
      )
      legend_l.append(pprof)
    plots[0].legend(legend_l, loc=1, prop = FontProperties(size=10)) 
    plt.savefig("plot_dj.pdf")
    #plt.show()
